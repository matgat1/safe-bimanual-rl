...
class SafeReplayMemory(Serializable):
    """Replay memory that stores 7-tuples (s, a, r, s', cost, absorbing, last)."""
    ...
    def get(self, n_samples):
        ...
        return (
            np.array([self._states[i] for i in idxs]),
            np.array([self._actions[i] for i in idxs]),
            np.array([self._rewards[i] for i in idxs]),
            np.array([self._next_states[i] for i in idxs]),
            np.array([self._cost[i] for i in idxs]),
            np.array([self._absorbing[i] for i in idxs]),
            np.array([self._last[i] for i in idxs]),
        )

    def reset(self):
        self._idx = 0
        self._full = False
        self._states = [None] * self._max_size
        self._actions = [None] * self._max_size
        self._rewards = [None] * self._max_size
        self._next_states = [None] * self._max_size
        self._cost = [None] * self._max_size
        self._absorbing = [None] * self._max_size
        self._last = [None] * self._max_size
...
class ViolationReplayMemory(Serializable):
    """Replay memory for violation samples: 6-tuples (s, a, cost, s', absorbing, last)."""
    ...
    def add(self, dataset):
        for el in dataset:
            self._states[self._idx] = el[0]
            self._actions[self._idx] = el[1]
            self._cost[self._idx] = el[2]
            self._next_states[self._idx] = el[3]
            self._absorbing[self._idx] = el[4]
            self._last[self._idx] = el[5]
...
class SafeLayerReplayMemory(ReplayMemory):
    def __init__(self, initial_size, max_size):
        ...
        self._add_save_attr(_cost="pickle!", _prev_cost="pickle!")
...
class ConstraintBucketReplayMemory(Serializable):
    ...
    def add(self, dataset):
        ...
        if self._buckets is None:
            ...
            self._buckets = [
                self._min + i * bucket_size for i in range(1, self._num_buckets)
            ]
    ...
    def reset(self):
        ...
        self._states = [
            [None for _ in range(self._max_size_per_bucket)]
    ...
    def size(self):
        ...
        """
        return [
            self._idx[b] if not self._full[b] else self._max_size_per_bucket
