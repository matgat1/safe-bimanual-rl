...
class AccelerationControlSystem(ControlAffineSystem):
    """
    Second-order (acceleration) control system.
    State is [q, q_dot] so dim_q here is the actual DOF count,
    and the controllable state vector has size 2*dim_q.
    """

    def __init__(self, dim_q, index_q, acc_limit, **kwargs):
        if np.isscalar(acc_limit):
            self.acc_limit = np.ones(dim_q) * acc_limit
        else:
            self.acc_limit = acc_limit

        self._add_save_attr(acc_limit="primitive")
        super().__init__(dim_q * 2, dim_q, index_q, **kwargs)
