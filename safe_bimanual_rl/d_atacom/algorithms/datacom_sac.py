...
class DatacomSACPolicy(Policy):
    ...
    def draw_action(self, state, policy_state=None):
        action = self.compute_action_and_log_prob_t(
            np.atleast_2d(state), return_log_prob=False
        )
    ...
    def apply_atacom(self, alpha, state):
        ...
        action_scale = torch.minimum(
            torch.min(1 / torch.abs(action), axis=-1)[0],
    ...
    def J_slack(self, slack):
        ...
        np.einsum("ijj->ij", out)[:] = (
            1 / np.maximum(np.exp(-self._atacom_beta() * slack), 1e-5) - 1
        )
    ...
    def _get_atacom_transformations(self, alpha, state):
        ...
        cons, J_k, J_x = self.compute_constraint_and_grad(
            q, x
        )  # (B, k), (B, k, q), (B, k, x)
        ...
        if self._analytical_const:
            ...
            anal_J_k_q_dot = (
                anal_J_k_q - anal_J_k_delta[:, :, :state_dim]
            ) / self.derivation_step_size
        ...
        # CAST ABACK TO TORCH
        return torch.from_numpy(B_u).to(self.device), torch.from_numpy(b).to(
            self.device
        )
    ...
    def compute_constraint_and_grad(self, q, x):
        ...
        return (
            ...
            J_q.double().cpu().numpy(),
            J_x.double().cpu().numpy(),
        )
    ...
    def compute_action_and_log_prob_t(self, state, return_log_prob=True):
        ...
        if return_log_prob:
            ...
            log_prob -= torch.log(1.0 - a.pow(2) + self._numeric_eps).sum(dim=1)
    ...
    @property
    def use_cuda(self):
        return TorchUtils.get_device().type == "cuda"
...
class DatacomSAC(DeepAC):
    ...
    def __init__(
        ...
        lr_delta,
        init_delta,
        delta_warmup_transitions,
        analytical_constraint=None,
        ...
        self._cost_budget = (
            cost_budget
            / mdp_info.horizon
            * (1 - mdp_info.gamma**mdp_info.horizon)
            / (1 - mdp_info.gamma)
        )
        ...
        if target_entropy is None:
            self._target_entropy = -np.prod(mdp_info.action_space.shape).astype(
                np.float32
            )
        else:
        ...
        self._violation_replay_memory = ViolationReplayMemory(0, max_replay_size // 10)
        ...
        self._constraint_approximator = Regressor(
            TorchApproximator, **constraint_params
        )
        self._target_constraint_approximator = Regressor(
        ...
        self._init_target(
            self._constraint_approximator, self._target_constraint_approximator
        )

        policy = DatacomSACPolicy(
            actor_mu_approximator,
            actor_sigma_approximator,
            self._constraint_approximator,
            control_system,
            accepted_risk,
            self.delta,
            atacom_lam,
            ...
            log_std_min,
            log_std_max,
            analytical_constraint,
        )

        self._log_alpha = torch.tensor(0.0, dtype=torch.float32)
        self._delta_value = torch.tensor(
            np.maximum(init_delta, 0.1), dtype=torch.float32
        )
    ...
    def fit(self, dataset, **info):
        ...
        if (
            dataset[-1][-1]
            and self._replay_memory.size > self._delta_warmup_transitions
        ):
            self.update_delta()
        ...
        if self._replay_memory.initialized:
            state, action, reward, next_state, cost, absorbing, _ = (
                self._replay_memory.get(self._batch_size())
            )

            if self._replay_memory.size > self._warmup_transitions():
                action_new, log_prob_new = self.policy.compute_action_and_log_prob_t(
                    state
                )
            ...
            if self._violation_replay_memory.size > self._batch_size() // 10:
                (
                    ...
                    _,
                    violating_cost,
                    violating_next_state,
                    violating_absorbing,
                    _,
                ) = self._violation_replay_memory.get(self._batch_size() // 10)
            ...
            constraint_state, next_constraint_state = self.to_constraint_state(
                state, next_state
            )
            ...
            # Unsqueeze to (N, 1) to match constraint network output shape (N, 1)
            error = torch.tensor(
                np.maximum(cost, 0), dtype=torch.float32, device=self.device
            ).unsqueeze(-1)
            absorbing_t = torch.tensor(
                absorbing, dtype=torch.float32, device=self.device
            ).unsqueeze(-1)
            gamma_t = torch.ones_like(error) * self.mdp_info.gamma

            state_tensor = torch.tensor(
                constraint_state, dtype=torch.float32, device=self.device
            )
    ...
    def episode_start(self, initial_state=None, episode_info=None):
        policy_state, theta = super().episode_start(initial_state, episode_info)
        self._episode_end = True
        return policy_state, theta

    def update_delta(self):
        epi_states, _ = self.to_constraint_state(
            self._episode_states, self._episode_states
        )
        c_epi = (
            self.policy.compute_constraint(torch.tensor(epi_states).to(self.device))
            .detach()
            .cpu()
            + self.delta().detach()
            - self.delta()
        ...
        loss = F.smooth_l1_loss(
            cum_costs - self._cost_budget, c_epi.flatten(), reduction="mean"
        )
    ...
    def gaussian_wasserstein_dist(
        ...
        target_var = (
            1 - absorbing
        ) * target_var_not_absorbing + absorbing * target_var_absorbing

        J_sigma = F.mse_loss(
            predicted_cost[1].exp(), torch.sqrt(target_var), reduction=reduction
        )
