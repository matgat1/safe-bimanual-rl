...
if __name__ == "__main__":
    ...
    def _compute_action():
        ...
        right_rot_err = np.cross(right_gripper_mat[:, 1], r_hy) + np.cross(right_gripper_mat[:, 2], -right_handle_mat[:, 0])
        left_rot_err = np.cross(left_gripper_mat[:, 1], l_hy) + np.cross(left_gripper_mat[:, 2], left_handle_mat[:, 0])
        ...
        q_dot_r = J_r.T @ np.linalg.solve(J_r @ J_r.T + LAM * np.eye(6), np.concatenate([KP_POS * rel_right, KP_ROT * right_rot_err]))
        q_dot_l = J_l.T @ np.linalg.solve(J_l @ J_l.T + LAM * np.eye(6), np.concatenate([KP_POS * rel_left, KP_ROT * left_rot_err]))
