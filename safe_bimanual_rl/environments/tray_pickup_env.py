...
class TrayPickUpEnv(BimanualTableEnv):
    ...
    def _create_info_dictionary(self, obs, action):
        ...
        info["tray_push_penalty"] = self._tray_push_penalty if self._tray_pushed() else 0.0
        info["position_bonus"] = self._success_position_reward if both_reached else 0.0
        info["orientation_bonus"] = self._success_orientation_reward if both_reached else 0.0
        return info
...
if __name__ == "__main__":
    ...
    right_ee_id      = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "right_hande_robotiq_hande_end")
    left_ee_id       = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "left_hande_robotiq_hande_end")
    ...
    right_grasp_site = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "right_grasp_target")
    left_grasp_site  = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "left_grasp_target")

    # DOF address for each arm joint (maps joint -> qvel index)
    right_dof_ids = np.array([model.joint(f"right_arm_A{i}").dofadr[0] for i in range(1, 8)])
    left_dof_ids  = np.array([model.joint(f"left_arm_A{i}").dofadr[0] for i in range(1, 8)])
    ...
    def _compute_action():
        ...
        r_hy = right_handle_mat[:, 1] * np.sign(np.dot(right_gripper_mat[:, 1], right_handle_mat[:, 1]))
        l_hy = left_handle_mat[:, 1]  * np.sign(np.dot(left_gripper_mat[:, 1],  left_handle_mat[:, 1]))
        # z-axis (approach): right gripper_z → -handle_x, left gripper_z → +handle_x
        right_rot_err = (np.cross(right_gripper_mat[:, 1], r_hy)
                       + np.cross(right_gripper_mat[:, 2], -right_handle_mat[:, 0]))
        left_rot_err  = (np.cross(left_gripper_mat[:, 1],  l_hy)
                       + np.cross(left_gripper_mat[:, 2],   left_handle_mat[:, 0]))

        # Jacobians (3 x nv each for position and rotation)
        Jp_r = np.zeros((3, nv)); Jr_r = np.zeros((3, nv))
        mujoco.mj_jacBody(model, data, Jp_r, Jr_r, right_ee_id)

        Jp_l = np.zeros((3, nv)); Jr_l = np.zeros((3, nv))
        ...
        action[RIGHT_ACT] = np.clip(q_dot_r, space.low[RIGHT_ACT], space.high[RIGHT_ACT])
